    ...
    module.controller('GnMdViewController', [
      ...
      function ($scope, $http, $compile, gnSearchSettings, gnSearchLocation,
          ...
          $scope.$watch('mdView.current.record', function () {
              ...
              var oldSchema = document.getElementById("schemaDotOrg");
              if (oldSchema) {
                  oldSchema.parentNode.removeChild(oldSchema);
              }

              //Load new schema scripts
              if (gnMdViewObj.current.record != null) {

                  gnMdViewObj.current.record.identifierVisible = [];

                  for (var i = 0; i < gnMdViewObj.current.record.identifier.length; i++) {

                      var idString = gnMdViewObj.current.record.identifier[i];

                      if (typeof idString !== "string") {
                          idString = JSON.stringify(idString);
                      }

                      if (idString.startsWith('{"@context"')) {
                          gnMdViewObj.current.record.identifierVisible[i] = false;

                          var scriptTag = document.getElementById("schemaDotOrg");

                          if (scriptTag) {
                              scriptTag.innerHTML = idString; //update existing script tag
                          }
                          else {
                              //create script tag
                              scriptTag = document.createElement('script');
                              scriptTag.id = "schemaDotOrg";
                              scriptTag.type = "application/ld+json";
                              scriptTag.innerHTML = idString;

                              document.getElementsByTagName('head')[0].appendChild(scriptTag);
                          }
                      }
                      else {
                          gnMdViewObj.current.record.identifierVisible[i] = true;
                      }
                  }
              }
